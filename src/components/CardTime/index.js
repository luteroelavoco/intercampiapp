...
export default function CardTime({
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      ...
      if (horario.length > 0) Proximo();
      setTime(date);
    }, 1000);
    return () => {
      clearTimeout(timeout);
    };
  }, [Proximo, horario.length, time]);
  ...
  function getAnterior(index) {
    if (index == 0) return horario.length - 1;
    else return parseInt(index) - 1;
  }

  function getSeguinte(index) {
    if (index == horario.length - 1) return 0;
    else return parseInt(index) + 1;
  }
    ...
    return (
      <div className="card">
        ...
        <div className="bottom">
          <label style={{ color: "black" }}>
            Anterior <br /> <span>{horario[anterior]}</span>
          </label>
          <label style={{ color: cor }}>
            Proximo <br />
            <span>{horario[proximo]}</span>
            <br /> <i className="far fa-clock"> {getFaltaEmHora(falta)}</i>
          </label>
          <label style={{ color: "black" }}>
            Seguinte <br /> <span>{horario[seguinte]}</span>
          </label>
        </div>
      </div>
    );
